...
create table public.org_members (
  ...
  role text not null,  -- now an FK to org_roles(org_id, key); see Permissions (RBAC) below
  created_at timestamptz not null default now(),
...
`orgs` and `org_members` need their own policies too — this is the one place `is_org_member` exists specifically to avoid RLS recursion on `org_members` itself. Members can see their org and their org's membership rows, and every user can see their own membership rows. `org_members` has **no client write policy**: memberships change only through the user-management RPCs and the `manage-users` Edge Function (§4 Users & Security Groups). The old "owner/admin can manage membership" policy let an admin make themselves owner or remove the owner. `orgs` has an update policy gated on `has_permission(id, 'settings.company')` for Company Info. Org creation still only happens via manual SQL (`docs/onboarding-new-client.md`); there is no insert policy.

**Permissions (RBAC)** (`20260817000000_roles_permissions.sql`): membership alone no longer authorizes writes. Every write checks a specific permission from the `app_permissions` catalog, held by the member's role (`org_roles` + `role_permissions`, see §4 Users & Security Groups):
- `has_permission(org_id, 'invoices.void')`: the one check. The owner role holds every permission implicitly; other roles hold what `role_permissions` grants. Inactive members hold nothing.
- `assert_permission(org_id, key)`: raises a friendly "You don't have permission to do this (Void invoices)" error (errcode 42501). Every SECURITY DEFINER RPC calls it where it used to call `is_org_member` (generated mechanically in `20260817000100_rpc_permission_checks.sql`). **A new RPC must call `assert_permission` with the right key, not `is_org_member`.**
- Table write policies use `has_permission(org_id, '<key>')` (items, customers, suppliers, master data, ledger accounts, purchase orders, orgs). Select policies stay `is_org_member`, **except the ledger**: `journal_entries`/`journal_lines` need `accounts.view`, so the financial reports and dashboard profit chart (SECURITY INVOKER) come back empty without it.
- `is_org_member()` / `org_role()` now also require `status = 'active'`, so deactivating someone locks them out of every select policy at once.
- `reset_org_data` stays owner-only (`org_role = 'owner'`), as does editing Security Groups.
- **Adding a permission**: insert an `app_permissions` row (module, label, `requires` dependencies), grant it to the default roles in `seed_default_roles` and to existing orgs, add the key to `PERMISSIONS` in `src/auth/permissions.ts`, then gate the RPC/policy and the UI.
...
orgs                    -- tenants -- org_id, name, slug, address, phone, email,
                        --   currency_symbol (default '$'), currency_code (default 'USD'), timezone (IANA,
                        --   nullable = UTC; set in Company Info -- see the Org timezone paragraph in §4)
org_members             -- user_id x org_id, role (FK -> org_roles(org_id, key)), full_name, status
                        --   ('active'|'inactive'), invited_by -- no client write policy (RPC/Edge Function only)
org_roles               -- org_id, key, name, description, is_system -- the org's "Security Groups"; seeded
                        --   per org (owner/admin/manager/accountant/cashier) by an AFTER INSERT trigger on orgs
role_permissions        -- org_id, role_key, permission_key -- the Security Groups matrix; the owner role has
                        --   no rows (it implicitly holds everything)
app_permissions         -- key, module, module_label, label, description, requires text[] -- GLOBAL catalog
                        --   (no org_id, read-only to clients); drives the matrix UI and dependency checks
...
sales_receipts          -- org_id, receipt_number, customer_id, status, payment_method, subtotal, total,
                        --   sale_date (org-local calendar date, trigger-stamped; reports/views use it, not created_at)
...
user_preferences        -- user_id (default auth.uid()), org_id, key, value jsonb -- small per-user UI settings
                        --   (first use: the dashboard Transactions Summary row layout), unique (user_id, org_id,
                        --   key); RLS = own rows only, in orgs you belong to. Read/written through
                        --   useUserPreference() (src/lib/userPreferences.ts), which caches in localStorage
...
**Dashboard insights** (`20260814120000_dashboard_insights.sql`, `src/features/dashboard/`): below the 11 KPI tiles (which the owner explicitly wants kept exactly as they are — add tiles, never replace them), the dashboard shows the following sections. All the RPCs below follow the report-RPC convention (plain SQL, stable, ordinary RLS). "Sales" everywhere means invoices by `issue_date` plus sales receipts by `created_at::date`, voids excluded.
- **Two 30-day trend charts** (Sales vs Collections; Gross Profit vs Net Income), backed by `dashboard_daily_series(org, start, end, p_tz)`. It returns one row per day, empty days included. The profit series come from the ledger with the same income/COGS/expense split as `profit_and_loss`, so the two always agree (verified).
- **A 7-day Transactions Summary** (columns = days, today highlighted, plus a 7-day total), backed by `daily_activity_summary(org, start, end, p_tz)`. It returns long-format `(metric, day, amount)` rows for a 14-type master list (invoices, sales receipts, quotations, credit memos, refunds, customer payments, deposits, purchase orders, supplier bills, supplier payments, expenses, inventory adjustments, fund transfers; plus a client-derived "All sales"). Each user picks, orders, and resets their rows via **Edit table** (`TransactionsSummaryCard.tsx`). The layout is saved to `user_preferences` under `dashboard.transactions_summary`; saving exactly the default deletes the row, so improved defaults reach everyone. The default is the original eight rows.
- **Account Balances:** every balance-sheet account, matching the old app. Click a row to expand this month's lines via `general_ledger`. Each line's +/− comes from the running balance, so it's sign-correct for liability accounts too.
- **Recent Transactions:** from `all_transactions`.
- **Charts:** `TrendChart.tsx` is hand-drawn SVG with no chart library. It measures its container with a ResizeObserver so text stays crisp, has a hover crosshair and tooltip, and spaces date labels by pixel width.
- **Local timezone** (`20260815000000_local_timezone_prefs_search.sql`): `dashboard_summary(org, p_tz)` and both RPCs above take the browser's IANA timezone (`localTimeZone()` in `src/lib/dates.ts`). Before this, they bucketed by UTC day, so "today" rolled over an hour late in the UK in summer and five hours late in Pakistan. `safe_timezone()` falls back to UTC for an unknown name. Each record's local day is decided per source:
  - **Real timestamps** (`sales_receipts.created_at`, plus `created_at` for the documents whose date the DB stamps itself: invoices, credit memos, supplier bills, quotations, POs) convert to the viewer's zone.
  - **Dates a person picked** (expense/refund/deposit/adjustment dates, manual journal entries) are used as picked.
  - **Payments:** the forms send a date-only value, stored as 00:00:00 UTC. `local_day(ts, tz)` keeps that as the picked date rather than shifting it.
  - **Journal entries:** `journal_entry_is_system_dated(reference_type)` marks the auto-posted types whose `entry_date` was UTC-stamped, so the profit chart converts only those.
  - **Consequence:** the dashboard can put a near-midnight record on a different day than the reports, which still bucket by the stored (UTC-stamped) date. See §10.
- **`sales_totals`** (an editable-sales-window idea the owner declined in favour of keeping the tiles) was dropped.

**Org timezone** (`20260816000000_org_timezone_document_dates.sql`): `orgs.timezone` (IANA name, validated by trigger; NULL = UTC, the old behaviour) is chosen in Settings → Company Info. It exists because Supabase runs in UTC, so every `current_date` default and `timestamp::date` cast was a UTC date: a document created between local midnight and UTC midnight was dated yesterday. Helpers: `org_timezone(org)`, `org_today(org)`, `org_local_date(org, ts)`.
- **Document dates:** the `default current_date` was dropped from every system-stamped date column (invoices/credit memos/supplier bills/quotations `issue_date`, plus expense/refund/deposit/adjustment/price-list dates). A `BEFORE INSERT` trigger, `fill_org_local_date('<column>')`, fills the column with `org_today()` when the caller leaves it NULL. A date the caller passes is always kept. **New system-dated columns should use the same trigger, never `default current_date`.**
- **Sales receipts** got a stored `sale_date`, filled the same way. `sales_lines` and `all_transactions` use it instead of `created_at::date`.
- **Journal entries:** `journal_entry_org_date` (a `BEFORE INSERT` on `journal_entries`) re-dates system-dated auto-postings (`journal_entry_is_system_dated(reference_type)`) whose `entry_date = current_date` to `org_today()`. That's one chokepoint instead of editing every posting RPC. Entries given a document's own date only differ from UTC `current_date` when that date is already org-local, so they're kept.
- **Reports and views:** `inventory_movement`, `purchases_by_supplier`, `all_transactions` (PO fallback date) and the `outstanding_invoices`/`outstanding_supplier_bills` `is_overdue` flags now use the org's calendar.
- **Payments:** `paid_at::date` is still correct, because payments carry a date-only value the user picked.
- **No backfill:** existing rows were deliberately not re-dated (append-only convention), and `sale_date` was backfilled with the UTC date the ledger already used.
- **Dashboard vs org timezone:** the dashboard still buckets by the *viewer's* timezone (§4 Local timezone), which matches whenever the viewer is in the org's timezone.

**Supplier-filtered item pickers** (Purchase Order form, New Supplier Bill): once a supplier is chosen, the item dropdowns list only items whose `items.supplier_id` is that supplier.
- **Show all items:** a checkbox under the Supplier field lifts the filter.
- **Clearing:** switching supplier, or unticking "Show all items", clears any line whose item isn't linked to the supplier, including its unit cost, and says how many lines it cleared.
- **Shared code:** the logic is in `src/features/items/supplierItems.tsx` (`supplierItemOptions`, `clearUnlinkedLines`, `SupplierItemsToggle`). Reuse it for any other supplier-scoped item picker.

**Barcode scanning & fast invoice entry**:
- **Camera scanner** (`src/components/ui/CameraScanner.tsx`): a modal that decodes barcodes and QR codes from the camera with `@zxing/browser`.
  - **Loading:** the library is dynamically imported, so it ships as a separate lazy chunk and never loads unless a scanner opens.
  - **Camera:** it prefers the rear camera (`facingMode: environment`) and stops the camera on the first read.
  - **Feedback:** it beeps (`src/lib/beep.ts`, a Web Audio oscillator with no sound file) and vibrates on phones. Callers must call `primeBeep()` in the click that opens it, because browsers only allow audio after a user gesture.
  - **Errors:** denied permission, no camera, camera in use, and non-https each get a plain-language message and a Try again button.
  - **Where it's used:** `BarcodeField` (`src/features/items/BarcodeField.tsx`, the Barcode input plus a Scan button on New Item and Edit Item) and New Invoice's Camera button.
- **Autocomplete** (`src/components/ui/Autocomplete.tsx`) is the shared searchable picker, an ARIA combobox matching a case-insensitive substring anywhere in each option's search text.
  - **Select mode** is bound to `value`. Editing the text un-chooses the option, and blurring restores the last valid label.
  - **Action mode** (`clearOnPick`) is a command box. Picking calls `onPick`, then empties and refocuses the box, and Enter never submits the form.
  - **`resolveExact`** makes Enter take an exact code (barcode/SKU) match first. That's how a USB scanner (fast typing + Enter) lands on the right item even when other items contain the same digits.
  - **Portal:** the option list is portalled to `<body>` with fixed positioning, because line-item rows live inside an `overflow-x-auto` wrapper that would otherwise clip it. It repositions on scroll and resize.
- **New Invoice** uses Autocomplete for the customer (by name; "+" quick-add kept) and each line's item (by name, SKU, or barcode). A **"Scan or search to add items"** bar sits above the lines, with a "Location for new lines" picker and a Camera button.
  - **Adding:** a scanned or picked item that's already on the invoice gets quantity +1. Otherwise it fills the first empty line, or appends a new line at quantity 1 with the item's price and the default location.
  - **Feedback:** a status line confirms each add ("Added X", "X: quantity now N", "No item matches…"), and focus returns to the bar for the next scan.
- **Testing the camera path headlessly:** Chrome's `--use-fake-ui-for-media-stream --use-fake-device-for-media-stream --use-file-for-fake-video-capture=<file>.y4m` plays a Y4M file as the webcam. A QR frame generated with `@zxing/library`'s `QRCodeWriter` (the JS package has no 1D encoders) exercises the full scan → beep → fill path.

**Global search** (header, `src/components/layout/GlobalSearch.tsx`, RPC `global_search(org, query, limit)`), modelled on the original HashirHub top-bar search:
- **Matches:** 2+ characters, a case-insensitive substring match with LIKE wildcards escaped. It searches documents (via `all_transactions`: number or party name), customers (name/phone/email), suppliers, and items (name/SKU/barcode).
- **Results:** ranked documents first (newest first), then people, then items.
- **Keyboard:** Ctrl/Cmd+K or "/" focuses it, arrows move, Enter opens, Esc clears then closes. It's an ARIA combobox.
- **Phone:** it collapses to an icon that opens a full-width bar.
- **Destinations:** documents open their detail page (list page for expenses and refunds). Customers and suppliers open their statement. Items open Item Search with `?q=<sku>&item=<id>`, which pre-fills the search and selects the item.

**Printing** (`src/features/printing/`): invoices and sales receipts print from a **Print** button on their detail pages (`/invoices/:id/print`, `/sales/:id/print`), and many invoices at once from **Reports → Invoice Batch Print** (`/reports/invoice-batch-print`).
- **Batch filters:** invoice date range or invoice-number range. The number range is zero-padded `INV-000123` string bounds, since `next_document_number` always pads to 6 digits. Customer and unpaid-only filters are optional too. Voids are excluded, and there's a `BATCH_LIMIT` of 200 with a warning when it's hit.
- **Shared layout:** one `PrintableDocument` A4 layout serves every document. It shows Company Info from `orgs`, bill-to/sold-to details, lines, subtotal/total/paid/balance due, notes, and a VOID watermark on void documents. `api.ts` normalizes invoice and receipt rows into one `PrintableDoc` shape.
- **Print mechanics:** printing reuses the app's `print:` chrome-hiding. `.print-doc` in `index.css` keeps badge colours in print (`print-color-adjust: exact`) and starts every document after the first on a new page. "Save as PDF" in the browser dialog is the PDF path; there is no server-side PDF.
- **Adding a document type** (quotations, credit memos, ...): add a loader that maps to `PrintableDoc`, a title in `PrintableDocument`'s `TITLES`, and a route.
...
- **Adding users** goes through the `manage-users` Edge Function, the only code holding the service-role key (auto-provided by Supabase, never in the repo):
  ...
  - Someone who already has a HashirHub login (e.g. in another org) is just given a membership, and their password is left alone.
  - It also resends invites, and sets a password (`set_password`). It refuses the password change if the target belongs to any other org, because an admin here must not be able to take over a login used elsewhere.
  - Supabase's built-in SMTP sends only a few emails per hour, which is why the password option exists.
- **Guardrails (in SQL, not the UI):**
  - You need `users.manage` to invite, change or remove anyone.
  - A non-owner can't assign the Owner role, or any role holding a permission they lack (`assert_can_assign_role`). They also can't change an owner (`assert_can_manage_member`).
  - Nobody can change their own role or status, or remove themselves.
  - The last active owner can't be demoted, deactivated or removed.
  - Deactivating (status `inactive`) keeps the membership but blocks all access immediately. The locked-out user sees "Your access is turned off" because they can still read their own row.
- **Frontend enforcement** mirrors the database:
  - `OrgProvider` loads `my_permissions(org)` and exposes `can(key)`, `isOwner` and `useCan()`.
  - `src/auth/permissions.ts` holds the `PERMISSIONS` key list and `ROUTE_RULES` (path to requirement; first match wins, so `/x/new` comes before `/x/*`).
  - `canAccessPath()` is shared by the sidebar (it hides links and empty groups), the `AppLayout` route guard (it shows a "You don't have access" card), global search results, and the dashboard's recent transactions.
  - Report pages take their permission from `catalog.ts` (`permission` on each category, overridable per report).
  - Action buttons (New X, Void, Record payment, Convert to Bill, the "+" quick-adds, Edit) check `can(...)` inline. **Any new page must add a `ROUTE_RULES` entry, and any new action button must check the permission its RPC asserts.**
  - The dashboard hides the profit chart and Account Balances without `accounts.view`.
- **Known limits:**
  - Read access is enforced in the database only for the ledger. Other documents (invoices, bills...) stay readable by any active member through the API, and are hidden by the UI only. Most reports cut across modules, so per-module read RLS would break them.
  - `list_org_members` needs `users.manage`, because it joins `auth.users` for emails.
  - Price Manager users (`items.prices` without `items.edit`) can only change `unit_price`/`reorder_threshold`. The `items_guard_price_only_update` trigger enforces this.
  - The old direct `stock_movements` insert policy was dropped: every stock change now needs its document RPC.
- **Testing** used throwaway `@example.com` users made with the password method (no emails sent), then deleted from `auth.users`. Never invite fake addresses: bounces count against the project's email reputation.
...
│   ├── functions/manage-users/    -- Edge Function: invite/create logins (service role lives only here)
│   └── config.toml
├── src/
...
│   ├── lib/dates.ts               -- ymd/todayYmd/addDaysYmd (LOCAL calendar dates) + localTimeZone() -- see §9
│   ├── lib/userPreferences.ts     -- useUserPreference(key): per-user settings in user_preferences + localStorage cache
...
│   │   │                             layout component
│   │   ├── users/                 -- Settings → Users and Security Groups (see §4)
...
│   │   ├── printing/              -- printable invoice/receipt documents, single + batch print pages
...
- **`cn()` is plain clsx, not tailwind-merge** — two classes for the same property (`p-5` + `p-0`, `text-text` + `text-danger-600`) both apply, and stylesheet order, not intent, decides the winner. Primitives therefore never set a default the caller is expected to override: `CardBody` drops its default `p-5` on whichever axis the caller pads itself (`p-0` gives an edge-to-edge table card), and table/report cells set no default text colour.
...
- **Calendar dates are local, never UTC.** Never build a `YYYY-MM-DD` with `date.toISOString().slice(0, 10)`. It's UTC, so after local midnight east of UTC (the UK in summer, Pakistan) it gives yesterday. Use `ymd()` / `todayYmd()` / `addDaysYmd()` from `src/lib/dates.ts`; all 16 form date defaults were converted. Any RPC that buckets records by day must take a `p_tz` argument, be passed `localTimeZone()`, and decide each record's local day with the rules in §4 (Local timezone).
...
- **Tile liquid** (`src/components/ui/LiquidFill.tsx`, replaced the static `CornerWave` SVG): a small canvas holding a pool of tinted "liquid" across the bottom ~quarter of each dashboard stat tile.
  - **Look:** drawn in `currentColor`, so the caller picks the hue with a text colour class; the dashboard passes each tile's icon `text-*` tone. There are two translucent layers (a faint back wave, and a front wave with a depth gradient) plus a thin surface highlight, all at low alpha so the numbers over it stay readable.
  - **Motion:** at rest, a slow travelling ripple. The slosh is two damped springs: a tilt mode (~1.45 Hz) and a centre mode (~2.6 Hz). Pointer events on the parent card kick them: entering pushes the liquid away from the entry side, moving across rocks it, clicking (or tapping) splashes it. It settles within ~1.5s. Tune it with the constants and impulses at the top of the file.
  - **Cost:** one rAF loop per tile (11 on the dashboard). It pauses off screen (IntersectionObserver) and in hidden tabs, and is capped at 2x DPR. Measured at a steady 60fps with no long tasks while all tiles animate. `prefers-reduced-motion` draws a still surface and ignores the pointer. It's hidden in print.
  - **Wiring:** the card must be `relative overflow-hidden` (the canvas listens on its parent element), and the card's content `relative z-10` so it paints above the liquid.
...
- *(Resolved by Org timezone, kept for history)* **Known follow-up: system-stamped document dates are still UTC.** `create_invoice`/`create_credit_memo`/`create_supplier_bill`/`create_quotation` default `issue_date` to `current_date`, and several auto-postings use `current_date` for `entry_date`. Both are UTC in Supabase, so a document created between local midnight and UTC midnight gets yesterday's date. That affects its printed date, `all_transactions`, and every report; only the dashboard compensates. The proper fix is an org timezone setting (Company Info) that those RPCs use instead of `current_date`, plus the same `p_tz` treatment for the report RPCs.
- **Explicitly deferred to later phases**: linking Credit Memos to specific invoices (currently an immediate, aggregate AR reduction only — and credit memo returns still don't reverse COGS or touch `avg_cost`, matching Credit Memos' deliberately simple scope), converting a Quotation into an Invoice/Sales Receipt (currently a fully standalone estimate), a Posting Settings screen for explicit default-account configuration (currently auto-created by fixed name on first use instead, see §4's caveat about name collisions with manually-created accounts), wholesale/multi-tier pricing, the old-app reports still blocked on a missing feature (listed greyed-out in the Reports catalog: Sales by Representative / Return Stock by Representative / Sales by Salesman / Financial Recovery need sales reps, salesmen with commission, and customer credit managers; Customer & Item Discounts need line discounts; Sales Orders & Open Orders need a Sales Orders document; Deleted/Updated Transactions need an audit log; Damaged/Expired Inventory needs expiry/damage tracking), camera-based barcode scanning, multi-org switcher UI (schema supports multi-org membership already; Phase 1 UI just auto-selects a user's single org). (Camera scanning shipped for items and invoices, see above; New Sale could reuse `CameraScanner` the same way.)

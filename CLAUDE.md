...
- **Target look**:  (sic — the filename's spelling is the owner's) is a ChatGPT-made mockup of the dashboard that sets the visual direction: slate/indigo palette, soft floating sea-wave gradients (bottom-left of the layout, behind the sidebar, and a tinted wave in the bottom-right corner of each metric tile), styled after the NHS Aqua Theatre management system layout. Re-read it before any visual/theme work. It is **gitignored and local only** — it shows real account data (the owner's email, org data) and the repo is **public**, while everything in  is also served as-is on the live site. Never commit it or move it somewhere tracked; if a future session needs the design shared, describe it rather than publishing the screenshot.
- **Ambient waves** (): a fixed, full-width, ,  layer at  along the bottom of the viewport, rendered first inside . The content column is  and the sidebar is  with a translucent  so the swell shows through behind it on desktop (it stays an opaque drawer on mobile). The layer holds three stacked SVG waves (back/middle/front, colours  in the  block), each a 200%-wide path holding two identical periods, so the  keyframe (translateX 0 → −50%) loops seamlessly. An outer  keyframe adds a vertical bob. Timings are staggered 12s/16s/22s with alternating direction ( in ). All motion is transform-only CSS (no JS), frozen under , and hidden in print. A left-to-right mask keeps it strongest at the bottom-left. Any new full-page layout that should share the look must also keep its content above .
- **Tile corner waves** (): a static decorative SVG in the bottom-right of a card, drawn in  so the caller picks the hue with a text colour class (the dashboard passes each tile's icon  tone). The card must be , and its content  so it paints above the wave.

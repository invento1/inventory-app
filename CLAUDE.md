...
- **`item_last_purchase_price`**: items still have no stored cost — this view picks whichever is more recent between `purchase_order_lines.unit_cost` and `supplier_bill_items.unit_cost` per item (`distinct on (item_id) ... order by purchased_at desc`), surfaced as a hover tooltip (native `title` attribute) on the unit price field in New Sale / New Invoice so staff can gauge discount room against what was actually paid, without needing a real costing system.

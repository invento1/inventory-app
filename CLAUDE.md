...
- **Colours come only from the theme tokens** in `src/index.css`'s `@theme` block — `primary` (navy, primary buttons), `accent-*` (indigo: active nav, links, focus rings), `surface`/`surface-muted`/`border`/`text`/`text-muted`, and the semantic pairs `success`/`warning`/`danger`/`info` (`-50` pastel background, `-600` text/icon; `danger-700` for hover). Never use raw Tailwind palette classes (`bg-white`, `text-red-600`, `bg-black/40`, ...) — use `bg-surface`, `text-danger-600`, `bg-primary/40` — so a re-theme stays a one-file edit. Cards use `shadow-card`. `Badge` tones map onto the semantic pairs (`neutral` = `info`).

...
- **Two 30-day trend charts** (Sales vs Collections; Gross Profit vs Net Income), backed by `dashboard_daily_series(org, start, end)`. It returns one row per day, empty days included. The profit series come from the ledger with the same income/COGS/expense split as `profit_and_loss`, so the two always agree (verified).
- **A 7-day Transactions Summary** (rows = invoices, sales receipts, credit memos, refunds, customer payments, supplier bills, supplier payments, expenses; columns = days, today highlighted, plus a 7-day total), backed by `daily_activity_summary(org, start, end)`. It returns long-format `(metric, day, amount)` rows, which the client pivots.
...
- **Unused RPC:** `sales_totals(org, start, end)` exists (from an editable-sales-window idea the owner declined in favour of keeping the tiles) but isn't used by the UI.

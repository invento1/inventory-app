...
**Control account guardrails**: `Undeposited Funds`, `Accounts Receivable`, and `Accounts Payable` are sub-ledger-backed — their GL balance must always reconcile to `invoice_payments`/`deposits` (Undeposited Funds) or `invoices`/`supplier_bills` (AR/AP), and every legitimate posting to them already goes through a dedicated RPC that keeps that sub-ledger in sync. `create_journal_entry` (Fiscal Daybook) and `create_fund_transfer` (Banking) originally let a user pick *any* account, including these three — manually crediting `Undeposited Funds` there would zero out its GL balance without ever touching `invoice_payments.deposit_id`, leaving a payment stuck in the Record Deposit queue forever with no GL balance behind it (and the real deposit, if made later, would double-count into Cash/Bank). `ledger_accounts.is_control_account` (a trigger-maintained, one-directional flag — only ever set `true`, e.g. on rename, never cleared, so a renamed control account fails safe rather than reopening the bug) plus a plain `account_type in ('accounts_receivable','accounts_payable')` check are enforced by a shared `assert_accounts_not_controlled(account_ids uuid[])` helper, called only from `create_journal_entry` and `create_fund_transfer` — **never** from `post_journal_entry` itself, which stays completely unrestricted since every auto-posting RPC (`record_invoice_payment`, `record_deposit`, `create_invoice`, `record_supplier_bill_payment`, ...) legitimately posts to these exact accounts internally. Both screens also filter these three out of their account `<Select>`s client-side (`isManuallyPostable` in `src/features/accounts/api.ts`) as defense in depth, with an explanatory note pointing at Receive Payment/Pay Bills/Record Deposit instead.
